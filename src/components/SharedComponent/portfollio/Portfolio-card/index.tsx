          ...
          {portfolioinfo.map((item, index) => (
            <Link key={index} href={`/portfolio/${item.slug}`} passHref>
              <div className={`px-3 group ${index % 2 !== 0 ? 'lg:mt-24' : ''}`}>
                ...
                <h4 className='pb-0.5 pt-3 group-hover:text-primary group-hover:cursor-pointer text-sm font-semibold text-midnight_text dark:text-white'>
                  {item.title}
                </h4>
                <p className='text-secondary font-normal text-xs group-hover:text-primary group-hover:cursor-pointer dark:text-white/50'>
                  {item.info}
                </p>
              </div>
            </Link>
